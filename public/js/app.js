...
// XSS Protection — escape plain-text user input before innerHTML
...
class ExamApp {
    ...
    async init() {
        ...
        this.examId = urlParams.get('id') || urlParams.get('examId');
        ...
        if (this.isPreview) {
            ...
            banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999;background:linear-gradient(135deg,#f59e0b,#d97706);color:white;text-align:center;padding:0.5rem;font-weight:700;font-size:0.85rem;';
            banner.innerHTML = '👁 CHẾ ĐỘ XEM THỬ — Bài làm sẽ không được lưu <button onclick="window.close()" style="margin-left:1rem;background:rgba(255,255,255,0.2);border:none;color:white;padding:0.2rem 0.6rem;border-radius:6px;cursor:pointer;">✕ Đóng</button>';
    ...
    renderEssay(question) {
        ...
        this.instruction.innerHTML = instr ? renderMarkdown(instr) : '';
        this.instruction.style.display = instr ? '' : 'none';
        this.questionText.innerHTML = renderMarkdown(question.prompt || '');
        ...
        parts.forEach((part, i) => {
            ...
            if (i < parts.length - 1) {
                ...
                if (blank.type === 'dropdown' && blank.dropdownOptions && blank.dropdownOptions.length > 0) {
                    // Render <select> for dropdown type
                    const optionsHtml = blank.dropdownOptions.map(opt => {
                        const escaped = opt.replace(/"/g, '&quot;');
                        const sel = savedVal === opt ? 'selected' : '';
                        return `<option value="${escaped}" ${sel}>${opt}</option>`;
                    }).join('');
                    html += `<select class="fill-blank-input" data-blank-index="${blankIndex}"
                        style="display:inline-block;min-width:100px;max-width:200px;border:none;border-bottom:2px solid var(--primary,#6366f1);padding:0.15rem 0.4rem;background:transparent;font-size:inherit;font-family:inherit;color:inherit;outline:none;margin:0 0.2rem;text-align:center;cursor:pointer;"
                        onchange="window._examApp.saveFillBlank('${question.id}', ${blankIndex}, this.value)">
                        <option value="" ${!savedVal ? 'selected' : ''}>-- chọn --</option>
                        ${optionsHtml}
                    </select>`;
                } else {
                    // Render <input> for text/int/float types
    ...
    renderFreeForm(question) {
        ...
        if (question.showInstruction !== false && question.instruction) {
            this.instruction.innerHTML = renderMarkdown(question.instruction);
    ...
    async submitExam(auto = false) {
        ...
        if (this.isPreview) { alert('👁 Chế độ xem thử — bài làm không được nộp.'); return; }
        if (!auto) {
            // Count answered
