...
// ── Admin: unified submissions list across 3 IELTS skills ─────────
// Filters: { skill?, testId?, userId?, status? in 'all'|'completed'|'draft', limit }
async function listAllSubmissionsForAdmin({
    skill, testId, userId, status = 'all', limit = 100
} = {}) {
    const completed = status === 'completed' ? 'AND s.is_complete = true'
                    : status === 'draft'     ? 'AND s.is_complete = false'
                    : '';
    const filt = (alias = 's') => {
        const where = [];
        const params = [];
        if (testId) { params.push(testId); where.push(`${alias}.test_id = $${params.length}`); }
        if (userId) { params.push(userId); where.push(`${alias}.user_id = $${params.length}`); }
    ...
    if (!skill || skill === 'reading' || skill === 'listening') {
        const f = filt('s');
        queries.push(query(
            `SELECT 'reading'::text AS skill_kind,
                    s.id::text AS submission_id,
                    s.test_id::text AS test_id,
                    t.title AS title, t.skill::text AS test_skill,
                    s.user_id::text AS user_id,
                    u.username, u.display_name,
                    s.is_complete, s.band_score::float AS band, s.raw_score::float AS score,
                    s.hidden_by_user_at, s.created_at, s.submitted_at
               FROM ielts_submissions s
               JOIN ielts_tests t ON t.id = s.test_id
               LEFT JOIN users u ON u.id = s.user_id
              WHERE 1=1 ${completed} ${f.where}
              ${skill === 'reading' || skill === 'listening' ? `AND t.skill = '${skill}'` : ''}
    ...
    if (!skill || skill === 'writing') {
        const f = filt('ws');
        queries.push(query(
            `SELECT 'writing'::text AS skill_kind,
                    ws.id::text AS submission_id,
                    wp.test_id::text AS test_id,
                    t.title AS title, 'writing'::text AS test_skill,
                    ws.user_id::text AS user_id,
                    u.username, u.display_name,
                    ws.is_complete, ws.band_overall::float AS band, NULL::float AS score,
                    ws.hidden_by_user_at, ws.created_at, ws.submitted_at
              ...
              WHERE 1=1 ${completed.replace('s.', 'ws.')} ${f.where}
    ...
    if (!skill || skill === 'speaking') {
        const f = filt('ss');
        queries.push(query(
            `SELECT 'speaking'::text AS skill_kind,
                    ss.id::text AS submission_id,
                    sp.test_id::text AS test_id,
                    t.title AS title, 'speaking'::text AS test_skill,
                    ss.user_id::text AS user_id,
                    u.username, u.display_name,
                    ss.is_complete, ss.band_overall::float AS band, NULL::float AS score,
                    ss.hidden_by_user_at, ss.created_at, ss.submitted_at
              ...
              WHERE 1=1 ${completed.replace('s.', 'ss.')} ${f.where}
    ...
    return all.slice(0, limit).map(r => ({
        ...
        band: r.band,
        score: r.score,
        hiddenByUserAt: r.hidden_by_user_at,
    ...
    const rows = await query(
        `WITH unified AS (
            ...
            SELECT
             ...
             WHERE os.user_id = $1 AND os.completed_at IS NOT NULL AND os.hidden_by_user_at IS NULL
        )
        SELECT * FROM unified
...
async function myResultsStats(userId) {
    ...
    const rows = await query(
        `WITH all_completed AS (
            ...
            UNION ALL
            SELECT 'quiz'::text,     NULL::float, score::float, completed_at FROM open_submissions WHERE user_id = $1 AND completed_at IS NOT NULL AND score IS NOT NULL AND hidden_by_user_at IS NULL
        )
        SELECT kind,
               COUNT(*)::int      AS count,
...
module.exports = {
    ...
    finalizeSubmission, listSubmissions, listAllSubmissionsForAdmin,
    // Writing
