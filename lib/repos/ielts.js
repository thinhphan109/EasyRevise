...
async function myResultsStats(userId) {
    ...
    const rows = await query(
        `WITH all_completed AS (
            SELECT 'reading'::text AS kind, band_score::float AS band, NULL::float AS score, submitted_at FROM ielts_submissions WHERE user_id = $1 AND is_complete = true AND band_score IS NOT NULL
            UNION ALL
            SELECT 'writing'::text,  band_overall::float, NULL::float, submitted_at FROM ielts_writing_submissions WHERE user_id = $1 AND is_complete = true AND band_overall IS NOT NULL
            UNION ALL
            SELECT 'speaking'::text, band_overall::float, NULL::float, submitted_at FROM ielts_speaking_submissions WHERE user_id = $1 AND is_complete = true AND band_overall IS NOT NULL
            UNION ALL
