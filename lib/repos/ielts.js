...
// ── Tests ─────────────────────────────────────────────────────────────
async function listTests({ skill, module: mod, isPublished, limit = 100 } = {}) {
    const where = [];
    const params = [];
    if (skill)        { params.push(skill);   where.push(`skill = $${params.length}`); }
    if (mod)          { params.push(mod);     where.push(`module = $${params.length}`); }
    ...
    const rows = await query(
        `WITH unified AS (
            ...
            SELECT
             ...
             WHERE s.user_id = $1 AND s.is_complete = true
            ...
            SELECT
             ...
             WHERE ws.user_id = $1 AND ws.is_complete = true
            ...
            SELECT
             ...
             WHERE ss.user_id = $1 AND ss.is_complete = true
        )
        SELECT * FROM unified
...
async function myResultsStats(userId) {
    ...
    const rows = await query(
        `WITH all_completed AS (
            SELECT 'reading'::text AS kind, band_score AS band, submitted_at FROM ielts_submissions WHERE user_id = $1 AND is_complete = true AND band_score IS NOT NULL
            UNION ALL
            SELECT 'writing',  band_overall, submitted_at FROM ielts_writing_submissions WHERE user_id = $1 AND is_complete = true AND band_overall IS NOT NULL
            UNION ALL
            SELECT 'speaking', band_overall, submitted_at FROM ielts_speaking_submissions WHERE user_id = $1 AND is_complete = true AND band_overall IS NOT NULL
        )
        SELECT kind, COUNT(*)::int AS count, AVG(band)::float AS avg_band, MAX(band)::float AS max_band
          FROM all_completed
         GROUP BY kind`,
        [userId]
    );
    const out = { reading: null, writing: null, speaking: null, total: 0 };
    for (const r of rows) {
        out[r.kind] = { count: r.count, avgBand: r.avg_band, maxBand: r.max_band };
        out.total += r.count;
    }
    return out;
}
...
module.exports = {
    listTests, getTestById, createTest, updateTest, deleteTest,
